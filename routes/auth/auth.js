    ...
    _this.db.User.findOne({ where: { email: req.body.email }, include: [ "Role" ]}).then(function(user) {
      ...
      try {
        _this.pwcrypt.verify(user.salt, user.password_hash, req.body.password, (err, valid) => {
          if (err) return res.status(500).json({ success: false, errors: [{ type: 'auth.authenticate.password.invalid', message: 'An error occurred decrypting the password.' }]});
          ...
          return res.status(200).json({ success:true, message: 'Authenticated successfully.', response: { token: token, role: user.Role }});
        });
      } catch(err) {
